...
if (isset($_POST['allexp'])) {
    ...
    $graphdata = array();
    $year = date('Y') - 1;
    $dr = mysqli_query($conn, "SELECT `type` FROM `finance_records` WHERE `crdr`='CR' GROUP BY `type`");
    ...
    $yearquery = mysqli_query($conn, "SELECT `year` FROM `finance_records` WHERE `crdr`='CR' GROUP BY `year`");
    ...
    $data .= '</select>
            ...
            <tbody>
            ';
    $newquery = mysqli_query($conn, "SELECT `month`,`year`,`type`,SUM(`amount`) as amount FROM `finance_records` WHERE `crdr`='CR' GROUP BY `type`");
    ...
    $graphquery = mysqli_query($conn, "SELECT `type` , SUM(amount) as sum  FROM `finance_records` WHERE `crdr`='CR' GROUP BY `type`");
...
if (isset($_POST['specexp'])) {
    ...
    $data .= '</select>
            ...
            <tbody>
            ';
            $newquery = mysqli_query($conn, "SELECT month,year,SUM(`amount`) as amount FROM `finance_records` WHERE `crdr`='CR' and `type`='".$subtype."' GROUP BY month,year");
    while ($thatsit = mysqli_fetch_array($newquery)) {
        $data .= '
        <tr>
        <td>' . $thatsit['year'] .'-'. $thatsit['month'] .'</td>
